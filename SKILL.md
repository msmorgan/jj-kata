...
- Close your work before integrating: `integrate` refuses (exit `69`) unless the workspace's `@` is an EMPTY, undescribed change. Finish with `jj commit -m …` (or `jj describe -m …` then `jj new`) — it folds only commits you closed and never promotes the working copy for you.
- A conflicting `refresh` (exit `69`) is **routine, not a broken state** — trunk moved while you worked. Run `scripts/workflow resolve` from inside the workspace: it walks the conflicted stack **oldest-first**, so one edit often clears several commits. Do NOT follow jj's own `jj new`/`jj squash` hint (printed just above the toolkit's) — it fixes the commit you're standing on and leaves older ones conflicted.
- On divergence or other genuinely wrong state, run `scripts/workflow repair` from inside the feature workspace and reason through it step by step — **agent-initiated**, the toolkit never auto-runs repair. Both it and `resolve` drop you onto the conflict and print the exact marker locations as `file:line` hits (e.g. `…/f.txt:12:<<<<<<< conflict 1 of 2`); Read those lines, remove every marker, re-run until exit 0.
- Resolve alphabetized-list conflicts with `scripts/conflicts auto`; inspect any conflict with `scripts/conflicts show`.
- A `HANDOFF.md` in a workspace means work was paused there and left a resume doc; `scripts/workflow handoffs` scans every workspace for one (exit 0 = found, 1 = none). Writing and resuming one is the handoff skill's job — it's burn-after-reading, and you never adopt someone's paused work without asking first.
