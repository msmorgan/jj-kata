...
$tk/install.fish $coord >/dev/null 2>&1; or begin
    echo >&2 "smoke-handoff: install failed"
    exit 1
end
jj commit -m "install toolkit" >/dev/null 2>&1; or begin
...
set -l out (./scripts/workflow handoffs 2>/dev/null)
set -l rc $status
...
for name in feat-a feat-b feat-c
    ./scripts/workflow start $name >/dev/null 2>&1; or begin
...
echo $out[1] | string match -q "feat-a	$work/feat-a/HANDOFF.md"
...
pushd $work/feat-b
set out (./../myproj/scripts/workflow handoffs 2>/dev/null)
set rc $status
popd
...
rm $coord/HANDOFF.md $work/feat-c/HANDOFF.md
pushd $work/feat-a
...
./scripts/workflow handoffs feat-a >/dev/null 2>&1
test $status -eq 2; or _fail "handoffs accepted a NAME argument"
./scripts/workflow handoffs --help >/dev/null 2>&1
