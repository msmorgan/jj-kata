...
function _start_integrate --argument-names name
    $wf start $name >/dev/null 2>&1; or return 1
    $wf integrate $name >/dev/null 2>&1; or return 1
end
...
test -d $ws/feat-a; or begin; echo >&2 "smoke-sweep: guard refusal still deleted feat-a"; exit 1; end
echo "ok: --integrated argparse guards refuse bad combos"

# --- dry-run: reports, deletes nothing. ---
$wf drop --integrated --dry-run >/dev/null 2>&1; or begin
    echo >&2 "smoke-sweep: dry-run exited non-zero"
    exit 1
end
for d in feat-a feat-b feat-c feat-d
    test -d $ws/$d; or begin
        echo >&2 "smoke-sweep: dry-run deleted $ws/$d"
        exit 1
    end
end
...
$wf drop --integrated >/dev/null 2>&1; or begin
    echo >&2 "smoke-sweep: drop --integrated exited non-zero"
    exit 1
end
test ! -e $ws/feat-a; or begin; echo >&2 "smoke-sweep: feat-a survived the sweep"; exit 1; end
test ! -e $ws/feat-b; or begin; echo >&2 "smoke-sweep: feat-b survived the sweep"; exit 1; end
test -d $ws/feat-c; or begin; echo >&2 "smoke-sweep: feat-c (un-integrated) was swept"; exit 1; end
test -d $ws/feat-d; or begin; echo >&2 "smoke-sweep: feat-d (resumed work) was swept"; exit 1; end
